...
exports.__experimentalUseGlobalBehaviors = __experimentalUseGlobalBehaviors;
exports.__experimentalUseHasBehaviorsPanel = __experimentalUseHasBehaviorsPanel;
...
function __experimentalUseGlobalBehaviors(blockName, source = 'all') {
  const {
    merged: mergedConfig,
    base: baseConfig,
    user: userConfig,
  ...
  switch (source) {
    case 'all':
      rawResult = (0, _object.getValueFromObjectPath)(mergedConfig, finalPath);
      result = (0, _utils.getValueFromVariable)(mergedConfig, blockName, rawResult);
      break;
    case 'user':
      rawResult = (0, _object.getValueFromObjectPath)(userConfig, finalPath);
      result = (0, _utils.getValueFromVariable)(mergedConfig, blockName, rawResult);
    ...
    case 'base':
      rawResult = (0, _object.getValueFromObjectPath)(baseConfig, finalPath);
      result = (0, _utils.getValueFromVariable)(baseConfig, blockName, rawResult);
      break;
    default:
      throw 'Unsupported source';
  }
  ...
  const setBehavior = newValue => {
    ...
    setUserConfig(currentConfig => (0, _object.setImmutably)(currentConfig, finalPath.split('.'), newBehavior));
  };
  let behavior = '';
...
function __experimentalUseHasBehaviorsPanel(settings, name, {
  ...
  const hasSomeBlockSupport = Object.keys(settings?.behaviors).some(key => (0, _blocks.hasBlockSupport)(name, `behaviors.${key}`));
  if (blockSupportOnly) {
